...
//        Scanner scanner = new Scanner(new FileInputStream("C:\\Users\\GVoichuk\\IdeaProjects\\Test\\src\\main\\java\\com\\tinkoff\\trening\\input7full.txt"));
        ...
        List<Graph> set = new LinkedList<>();
        Graph invalidGraph = null;
        for (int i = 1; i <= n; i++) {
            int a = scanner.nextInt();
            Graph merged = null;

            if (a == i && invalidGraph != null){
                invalidGraph = new Graph(i, a);
                break;
            }


            if (set.isEmpty()){
                set.add(new Graph(i, a));
            } else {
                boolean added = false;
                for (Graph graph : set) {
                    if ((graph.first == a && graph.second == i) || (i == n && graph.first != a)){
                            invalidGraph = new Graph(i, a);
                    }
                    if (i == graph.second){
                        graph.second = a;
                ...
                while(added){
                    ...
                    for (Graph graph : set) {
                        if (graph == merged){
                            continue;
                        } else if (graph.first == merged.second){
                            graph.first = merged.first;
//                            graph.first = merged.first;
                            set.remove(merged);
                            merged = graph;
                            added = true;
        ...
        System.out.println(set);
        System.out.println(invalidGraph);
        ...
        int pos = -1;
        int newCell = -1;
        if (invalidGraph != null && set.size() == 1){
            Graph valid = set.get(0);
            pos = invalidGraph.first;
            newCell = valid.first;
